...
func TestIsINIFormat(t *testing.T) {
	testCases := []struct {
		name     string
		input    string
		expected bool
		...
		{
			name:     "basic INI config",
			input:    basicConfigINI,
			expected: true,
		},
		{
			name:     "INI config with VirtualCenter section",
			input:    basicConfigVcenterSectionINI,
			expected: true,
		},
		{
			name:     "multi VC INI config",
			input:    multiVCDCsConfigINI,
			expected: true,
		},
		{
			name:     "basic YAML config",
			input:    basicConfigYaml,
			expected: false,
		},
		{
			name:     "YAML config with vcenter section",
			input:    basicConfigVcenterSectionYAML,
			expected: false,
		},
		{
			name:     "empty config",
...
`,
		...
		{
			name: "YAML config with leading whitespace",
			input: `
global:
  server: 0.0.0.0
`,
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			g := gmg.NewWithT(t)
			result := IsINIFormat([]byte(tc.input))
			g.Expect(result).To(gmg.Equal(tc.expected))
		})
	}
}
