...
var _ = Describe("Trusted CA bundle sync controller", func() {
	...
	AfterEach(func() {
		...
		if proxyResource != nil {
			...
			Expect(cl.Get(ctx, client.ObjectKeyFromObject(proxyResource), &v1.Proxy{})).To(MatchError(apierrors.IsNotFound, "Proxy should have been deleted"))
		}
		...
		for _, cm := range allCMs.Items {
			...
			Expect(cl.Get(ctx, client.ObjectKeyFromObject(cm.DeepCopy()), &corev1.ConfigMap{})).To(MatchError(apierrors.IsNotFound, "ConfigMap should have been deleted"))
		}
